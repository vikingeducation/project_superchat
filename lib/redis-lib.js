...
const ORDERING_SCORE = 1; // set all sorted set values with same score to ensure alphabetical ordering
...
const addSortedItem = (key, item) => {
	return new Promise((resolve, reject) => {
		client.zset(key, ORDERING_SCORE, item, (err, data) => {
			if (err) reject(err);
			resolve(data);
		});
	});
